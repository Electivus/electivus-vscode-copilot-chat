...
export class CopilotCliBridgeSpanProcessor implements SpanProcessor {
	...
	onEnd(span: ReadableSpan): void {
		...
		const invocationId = span.attributes['electivus.copilot.hook.invocation_id'];
		if (span.name.startsWith('hook ') && span.attributes['electivus.copilot.hook.type'] && typeof invocationId === 'string') {
