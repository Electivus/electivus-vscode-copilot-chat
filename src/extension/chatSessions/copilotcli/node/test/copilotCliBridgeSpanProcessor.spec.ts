...
describe('CopilotCliBridgeSpanProcessor', () => {
	...
	it('enriches SDK hook spans with stashed event data', () => {
		...
		bridge.onEnd(makeReadableSpan({
			...
			traceId: 'trace-abc',
			attributes: {
				'electivus.copilot.hook.type': 'sessionEnd',
				'electivus.copilot.hook.invocation_id': 'inv-123',
			},
		}));
	...
	it('holds SDK hook span until hook.end data arrives', () => {
		...
		bridge.onEnd(makeReadableSpan({
			...
			traceId: 'trace-abc',
			attributes: {
				'electivus.copilot.hook.type': 'preToolUse',
				'electivus.copilot.hook.invocation_id': 'inv-456',
			},
		}));
