...
const PENDING_CHAT_SESSION_STORAGE_KEY = 'electivus.copilot.pendingChatSession';
...
export class ChatSessionsUriHandler extends Disposable implements CustomUriHandler {
			...
			case UriHandlerPaths.OpenSession:
				{
					...
					*/
					...
					if (type?.startsWith('copilot') && prId) {
						...
						return this._openGitHubSession('electivus-copilot-cloud-agent', prId, url, branch);
					}
				}
		}
	}
	...
	public async openPendingSession(details?: {
		...
		const uri = vscode.Uri.from({ scheme: 'electivus-copilot-cloud-agent', path: '/' + pullRequest.number.toString() });
