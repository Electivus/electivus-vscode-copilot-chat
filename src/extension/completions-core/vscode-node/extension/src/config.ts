...
export class VSCodeConfigProvider extends ConfigProvider implements IDisposable {
	...
	override onDidChangeCopilotSettings: ConfigProvider['onDidChangeCopilotSettings'] = transformEvent(
		...
		event => {
			if (event.affectsConfiguration('electivus.copilot')) {
				return this;
			}
			if (event.affectsConfiguration('electivus.electivus-vscode-copilot-chat')) {
				return this;
			}
		}
	);
...
export class VSCodeEditorInfo implements ICompletionsEditorAndPluginInfo {
	...
	getEditorPluginInfo() {
		return { name: 'electivus-vscode-copilot-chat', readableName: 'Electivus Copilot Chat for Visual Studio Code', version: packageJson.version };
	}
	getRelatedPluginInfo() {
		...
		return [
			...
			'electivus.electivus-vscode-copilot-chat',
		]
			.map(name => {
...
export async function enableCompletions(accessor: ServicesAccessor) {
	...
	// The rest of this function is the inverse of disableCompletions(), updating the electivus.copilot.enable setting.
...
/**
 * Disable completions using the electivus.copilot.enable setting.
 */
