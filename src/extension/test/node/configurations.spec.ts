...
describe('Configurations', () => {
	...
	it('settings in code should match package.json', () => {
		...
		internalKeys.forEach(key => {
			expect(key, 'Internal settings must start with electivus.copilot.chat.advanced.').toMatch(/^electivus\.copilot\.chat\.advanced\./);
		});
		...
		advancedPublicKeys.forEach(key => {
			expect(key, 'Advanced settings must not start wih electivus.copilot.chat.advanced.').not.toMatch(/^github\.copilot\.chat\.advanced\./);
