...
const reviewDiffContextKey = 'electivus.copilot.chat.reviewDiff.enabled';
const reviewDiffReposContextKey = 'electivus.copilot.chat.reviewDiff.enabledRootUris';
const numberOfReviewCommentsKey = 'electivus.copilot.chat.review.numberOfComments';
...
export class ReviewServiceImpl implements IReviewService {
	...
	constructor(
		...
		this._disposables.add(vscode.workspace.onDidChangeConfiguration(e => {
			...
			if (e.affectsConfiguration('electivus.copilot.advanced') || e.affectsConfiguration('electivus.copilot.advanced.review.intent')) {
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = vscode.languages.createDiagnosticCollection('electivus.copilot.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (this._comments.length === 1) {
				vscode.commands.executeCommand('electivus.copilot.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					...
					if (active !== this._activeThread) {
						...
						if (active) {
							vscode.commands.executeCommand('electivus.copilot.chat.review.current', active);
						}
					}
				}, 500);
			}
		...
		const comments = [
			{
				...
				author: {
					...
					iconPath: URI.joinPath(this._contextService.extensionUri, 'assets', 'electivus-copilot-chat.png'),
				},
			}
		];
		return comments;
